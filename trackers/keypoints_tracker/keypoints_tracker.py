...
class Keypoints(Object):
    ...
    def draw(self, frame: np.ndarray) -> np.ndarray:
        """
        Draw court keypoints and lines to form a real padel court
        """
        ...
        # The keypoints are:
        # 0, 1: Bottom Baseline (k1, k2)
        # 2, 3, 4: Bottom Service Line (k3, k4, k5)
        # 5, 6: Center/Net Line (k6, k7)
        # 7, 8, 9: Top Service Line (k8, k9, k10)
        # 10, 11: Top Baseline (k11, k12)

        def get_point(kid):
            if kid in self.keypoints_by_id:
                return self.keypoints_by_id[kid].asint()
            return None
        ...
        court_lines = [
            ...
            (0, 10),  # Left Sideline
            (1, 11),  # Right Sideline
            (2, 4),   # Bottom Service Line
            (7, 9),   # Top Service Line
            (5, 6),   # Net Line
            (3, 8),   # Center Line
        ]
        ...
        for start_id, end_id in court_lines:
            pt_start = get_point(start_id)
            pt_end = get_point(end_id)
