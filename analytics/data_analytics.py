...
class DataAnalytics:
    ...
    def add_ball_position(self, position: tuple[float, float]):
        self.current_datapoint.ball_position = BallPosition(position=position)
    ...
    def into_dataframe(self, fps: int) -> pd.DataFrame:
        ...
        # Convert position columns from object (with Python None) to float64
        # so that NaN arithmetic works correctly in diff() and eval()
        for col in ["frame", "player1_x", "player1_y", "player2_x", "player2_y",
                    "player3_x", "player3_y", "player4_x", "player4_y", "ball_x", "ball_y"]:
