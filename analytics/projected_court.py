...
class ProjectedCourtKeypoints:
    ...
    def keypoints(
        ...
        # Statically define keys to guarantee correct ID mapping (k1=0, k2=1, ... k12=11)
        keys = ["k1", "k2", "k3", "k4", "k5", "k6", "k7", "k8", "k9", "k10", "k11", "k12"]
        
        keypoints_12 = [
            Keypoint(id=i, xy=tuple(float(p) for p in getattr(self, k)))
    ...
    def update_from_list(self, points: list):
        """
        Update keypoints from a list of 12 points
        """
        for i in range(12):
            setattr(self, f"k{i+1}", tuple(points[i]))
        self.origin = self._get_origin()
...
class ProjectedCourt:
    ...
    def homography_matrix(self, keypoints_detection: Keypoints) -> np.ndarray:
        ...
        keypoints_detection = list(keypoints_detection.keypoints)
        
        if len(keypoints_detection) == 12:
            keypoints_detection.sort(key=lambda x: x.id)
            
            src_points = np.array([kp.xy for kp in keypoints_detection], dtype=np.float32)
            dst_keypoints = self.court_keypoints.keypoints(number_keypoints=12)
            dst_points = np.array([kp.xy for kp in dst_keypoints], dtype=np.float32)
    ...
    def draw_projected_ball(
        ...
        if data_analytics is not None:
            shifted_pos = self.court_keypoints.shift_point_origin(
                point=tuple(float(v) for v in projected_point),
                dimension="meters",
            )
            data_analytics.add_ball_position(position=shifted_pos)

        return ball_detection.draw_projection(frame)
    ...
    def draw_projections_and_collect_data(
        ...
        if self.H is None and keypoints_detection:
            self.H = self.homography_matrix(keypoints_detection)
        elif not is_fixed_keypoints and keypoints_detection:
            self.H = self.homography_matrix(keypoints_detection)
